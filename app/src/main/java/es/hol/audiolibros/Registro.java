...
public class Registro extends Activity {
   ...
   public void onCreate( Bundle savedInstanceState ) {
      ...
      URL_connect = "http://" + IP_Server + "/WebService/registro.php"/* ruta donde están nuestros archivos */;
      ...
      btn_regis.setOnClickListener( new View.OnClickListener( ) {
         public void onClick( View view ) {
            ...
            // Verificamos si no están en blanco.
            if ( checkregdata( usuario, passw, correo ) ) {

               // Ciframos la contraseña aplicándole una
               // máscara (la misma que la del WebService).
 ...
 * Podría hacerse lo mismo sin usar esto, pero si el tiempo de respuesta es demasiado lo que podría
 * ocurrir si la conexión es lenta o el servidor tarda en responder la aplicacion será inestable.
 * Además observaríamos el mensaje de que la app no responde.(ANR)
 */

   // Las peticiones http deben ejecutarse de una manera asíncrona, el motivo es porque a partir de
   // la 'versión 4 de android'v dará un error si estas peticiones se ejecutan en el hilo principal,
   // y esto tiene sentido, pues este tipo de sentencias pueden 'ralentizar la aplicación', aún así,
   // en las versiones 2.x de android funcionará si lo ejecutamos en el hilo principal. Para evitar
   // errores usaremos un proceso Asynctask para comunicarnos con el servidor.
