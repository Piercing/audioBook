...
// Esta clase se encarga de abrir la base de datos si existe o de crearla si no existe.
// Incluso de actualizar la versión si decidimos crear una nueva estructura de la base de datos.
// Para crear la base de datos crearemosuna subclasede SQLiteOpenHelper y sobreescribiremos el método
// onCreate(). Opcionalmente también se puede implemenar el método onUpgrade() y onOpen().
// El método onCreate()se utiliza para crear la BD la primera vez. En el proceso de creación usaremos
...
public class DataBase extends SQLiteOpenHelper {
  ...
  public void onCreate( SQLiteDatabase db ) {
    ...
    //crear tabla a partir de los datos de la variable CREATE_BDD
    try {
   ...
   */
  //-- INSERTAR
  //inserta una fila en la BD a partir de los datos de un libro, mediante el método insert().
...
//pasados como parámetros,mediante el método update().
  ...
  //actualiza los datos del libro identificado por numero, con los nuevos valores
  //pasados como parámetros,mediante el método update().
  public boolean actualizarLibro( int Id, String Titulo, String Autor, String Tema, String Mp3url, String Descrip,
                                  int Posicion, byte[]
                                      Imagen ) {
    SQLiteDatabase db = this.getWritableDatabase( );
...
//elimina el libro identificado por numero, mediante el método delete().
  ...
  //--CONSULTAR
//consulta a la BD para obtener todos los libros usando el método query().
